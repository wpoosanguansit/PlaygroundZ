...
class AgentSpec extends Specification {
  ...
  val A: CharCapitalSpacePound   = 'A'
  val B: CharCapitalSpacePound   = 'B'
  val C: CharCapitalSpacePound   = 'C'
  val D: CharCapitalSpacePound   = 'D'
  val E: CharCapitalSpacePound   = 'E'
  val F: CharCapitalSpacePound   = 'F'
  val G: CharCapitalSpacePound   = 'G'
  val H: CharCapitalSpacePound   = 'H'
  val P: CharCapitalSpacePound   = '#'
  val AA: CharCapital            = 'A'
  val BB: CharCapital            = 'B'
  val CC: CharCapital            = 'C'
  val DD: CharCapital            = 'D'
  val EE: CharCapital            = 'E'
  val FF: CharCapital            = 'F'
  val GG: CharCapital            = 'G'
  val HH: CharCapital            = 'H'
  ...
  val initial = Env(
    ...
    Dequeue.empty[Point],
    Dequeue(BB, CC, DD, EE, FF),
    Some(Point(1, 2)),
    true)

  def shouldHaveFoundH          = {
    val state   = processInputsZ(G, B, P, P, F)(initial)
    println(initial)
    val result  = runtime.unsafeRun(state)
    println(result)
    val state1  = processInputsZ(B, P, G, A, C)(result)
    val result1 = runtime.unsafeRun(state1)
    println(result1)
    val state2  = processInputsZ(A, H, P, P, B)(result1)
    val result2 = runtime.unsafeRun(state2)
    println(result2)
    val state3  = processInputsZ(H, P, A, P, P)(result2)
    val result3 = runtime.unsafeRun(state3)
    println(result3)
  ...
  def shouldHaveEnded           = {
    ...
    val result3 = runtime.unsafeRun(state3)
    println(result3)
    val state4  = processInputsZ(A, H, P, P, B)(result3)
    val result4 = runtime.unsafeRun(state4)
    println(result4)
  ...
  def shouldHaveCorrectDijkstra = {
    ...
    val result: Dequeue[Point] = initial.getPathFromCurrentToTarget
